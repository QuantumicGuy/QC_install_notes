...
#  variable text_orient  "0"; # orientation of the text label to the bar.
  ...
  variable scalecolor       RWB
  variable fpformat         "% 7.2f"
  #variable fpformat         1      # 0=decimal, 1=scientific
}

package provide colorscalebar $ColorScaleBar::version

proc ::ColorScaleBar::color_scale_bar {{length 0.5} {width 0.05} {auto_scale 1} {fixed 1} {min 0} {max 100} {label_num 5} {text 16} {scolor RWB} {fp_format "% 7.2f"} {x_pos -1.0} {y_pos -1.0} {replacebar 1} {molid top} {repid 0} {showlegend 0} {legend "Color Scale:"}} {
  variable bar_mol
  ...
  for {set i 0} {$i <= $label_num } { incr i 1} {
    ...
    set labeltxt [format $fp_format  $cur_text]
    #set labeltxt ""
    #if { $fp_format == 0 } {
...
proc ::ColorScaleBar::gui { } {
  ...
  # Molecule selector
  label $w.bar.mlabel -text "Molecule:" -anchor w
  ...
  label $w.bar.rlabel -text "Representation:" -anchor w
  ...
  label $w.bar.llabel -text "Colorbar-Length:"
  entry $w.bar.lentry -width 4 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::lengthsetting

  label $w.bar.wlabel -text "Colorbar-Width:"
  entry $w.bar.wentry -width 4 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::widthsetting

  label $w.bar.scaleMethod -text "ColorMethod:"
  eval [concat "tk_optionMenu $w.bar.scalechooser ::ColorScaleBar::scalecolor" \
            [colorinfo scale methods] ]
  ...
  label $w.bar.tlabel -text "Colorbar-Title:"
  ...
  label $w.bar.minlabel -text "Scale-Value Min:"
  entry $w.bar.minentry -width 4 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::minvalue

  label $w.bar.maxlabel -text "Scale-Value Max:"
  entry $w.bar.maxentry -width 4 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::maxvalue

  label $w.bar.xlabel -text "Labels-Number:"
  entry $w.bar.xentry -width 4 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::axislabels

  label $w.bar.clabel -text "Labels-Color:"
  eval [concat "tk_optionMenu $w.bar.cchooser ::ColorScaleBar::textcolor" \
            [colorinfo colors] ]
  label $w.bar.fpformatLabel -text "Label format:"
  entry $w.bar.fpformatValue -width 12 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::fpformat


  grid config $w.bar.llabel       -column 0 -row  0 -sticky w
  grid config $w.bar.lentry       -column 1 -row  0 -sticky w 
  
  grid config $w.bar.wlabel       -column 0 -row  1 -sticky w
  grid config $w.bar.wentry       -column 1 -row  1 -sticky w
  
  grid config $w.bar.scaleMethod  -column 0 -row  2 -sticky w
  grid config $w.bar.scalechooser -column 1 -row  2 -sticky w
  
  grid config $w.bar.dlabel   -column 0 -row  3 -sticky w
  grid config $w.bar.doff     -column 1 -row  3 -sticky w
  grid config $w.bar.don      -column 2 -row  3 -sticky w
  
  grid config $w.bar.tlabel   -column 0 -row  4 -sticky w
  grid config $w.bar.tentry   -column 1 -row  4 -sticky w
  
  grid config $w.bar.alabel   -column 0 -row  5 -sticky w
  grid config $w.bar.aoff     -column 1 -row  5 -sticky w
  grid config $w.bar.aon      -column 2 -row  5 -sticky w
  
  grid config $w.bar.mlabel   -column 0 -row  6 -sticky w
  grid config $w.bar.m        -column 1 -row  6 -sticky w
  grid config $w.bar.rlabel   -column 2 -row  6 -sticky e
  grid config $w.bar.r        -column 3 -row  6 -sticky w
  
  grid config $w.bar.minlabel -column 0 -row  7 -sticky w
  grid config $w.bar.minentry -column 1 -row  7 -sticky w

  grid config $w.bar.maxlabel -column 0 -row  8 -sticky w
  grid config $w.bar.maxentry -column 1 -row  8 -sticky w
  
  grid config $w.bar.xlabel   -column 0 -row  9 -sticky w
  grid config $w.bar.xentry   -column 1 -row  9 -sticky w
  
  grid config $w.bar.clabel            -column 0 -row 10 -sticky w
  grid config $w.bar.cchooser          -column 1 -row 10 -sticky w
  grid config $w.bar.fpformatLabel     -column 2 -row 10 -sticky e
  grid config $w.bar.fpformatValue     -column 3 -row 10 -sticky w
  ...
  #entry $w.labelformat.fpformatValue -width 12 -relief sunken -bd 2 \
    -textvariable ::ColorScaleBar::fpformat
  ...
  #radiobutton $w.labelformat.scientific -text "Scientific" -value "1" \
    -variable ::ColorScaleBar::fpformat
  #pack $w.labelformat.label $w.labelformat.fpformatValue -side left -anchor w -fill x

  button $w.drawcolorscale -text "Draw Color Scale Bar" \
      -command [namespace code {
      if { [color_scale_bar $lengthsetting $widthsetting $autoscale $fixedsetting $minvalue $maxvalue $axislabels $textcolor $scalecolor $fpformat -1.0 -1.0 1 $molid $repid  $text_show $bar_text] == -1 } { 
  ...
  pack $w.menubar $w.bar $w.drawcolorscale $w.delcolorscale -anchor w -fill x
