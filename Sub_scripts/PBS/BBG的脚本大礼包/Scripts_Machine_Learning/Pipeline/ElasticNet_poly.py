...
column_trans = DataFrameMapper(
                         [
                         (columns_list, [PolynomialFeatures(degree=2,include_bias=False), StandardScaler()])],
                          None
                            )
...
'''Pipeline parameters'''
pipe_params_tests =  [{ 'en__alpha':[0.05,0.08,0.09,0.1,0.12,0.15,0.2,0.5,1], 
                        'en__l1_ratio':[i/10 for i in range(1,10,1)],
                        'pf__degree':[2,3]}
                     ]
