...
def model_choice(model_name, model_params):
    ...
    if model_name == "MLPRegressor":
        ...
        print('** Please choose a MLP model **\n-> Supported MLP models are as follows:')
        print('   (1) MLPRegressor\n   (2) MLPClassifier')
        exit(1)
...
model_name = "MLPRegressor"  # "MLPRegressor" and "MLPClassifier"
model_params = {'random_state':0, 'max_iter':10000, 'solver':'lbfgs'}
...
tuple_combinations = [(i,i,i) for i in range(10,40,1)] +\
                     [(i,i,i,i) for i in range(10,40,1)] +\
                     [(i,i,i,i,i) for i in range(10,40,1)] +\
                     [(i,i,i,i,i,i) for i in range(10,40,1)] +\
                     [(i,i,i,i,i,i,i) for i in range(10,40,1)] +\
                     [(i,i,i,i,i,i,i,i) for i in range(10,40,1)] 
pipe_params = {}
pipe_params['mlpr__activation'] = optuna.distributions.CategoricalDistribution(['identity', 'logistic', 'tanh', 'relu'])
pipe_params['mlpr__alpha'] = optuna.distributions.LogUniformDistribution(1e-5,1e3)
pipe_params['mlpr__hidden_layer_sizes'] = optuna.distributions.CategoricalDistribution(tuple_combinations)
