...
def model_choice(model_name, model_params):
    ...
    if model_name == "VotingRegressor":
        ...
        model = VotingRegressor(**model_params)
        ML_type = "Regression"
        return "vr", model
        ...
        print('** Please choose a Voting model **\n-> Supported Voting models are as follows:')
        print('   (1) VotingRegressor\n   (2) VotingClassifier')
        exit(1)
...
'''model parameters'''
model_name = "VotingRegressor"  #  "VotingRegressor" or "VotingClassifier"
...
column_list = ['ZnO-2','DC-01','DC-01T','ZnO-1','WS','PCZ-70','SL-7025']

column_trans = DataFrameMapper(
                         [
                          (column_list, OneHotEncoder(categories=[ZnO2,DC01,DC01T,ZnO1,WS,PCZ70,SL7025]))
                          ],
                            None
                            )
...
best_reg1_params = {'random_state':0,'max_features': 6, 'min_samples_leaf': 3, 'min_samples_split': 2, 'n_estimators': 92}
reg1 = GradientBoostingRegressor(**best_reg1_params)

best_reg2_params = {'random_state':0,'max_features': 7, 'min_samples_leaf': 2, 'min_samples_split': 2, 'n_estimators': 94}
reg2 = RandomForestRegressor(**best_reg2_params)

pipe_params_tests =  [{
                        'vr__estimators':[[('gbr', reg1), ('rfr', reg2)]],
                        'vr__weights':[[round(i*0.1, 1), round(1 - i*0.1, 1)] for i in range(0,11,1)]
                        }
                     ]
