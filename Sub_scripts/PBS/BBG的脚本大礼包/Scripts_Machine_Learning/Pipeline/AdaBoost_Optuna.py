...
def model_choice(model_name, model_params):
    ...
    if model_name == "AdaBoostRegressor":
        ...
        print('** Please choose a AdaBoost model **\n-> Supported AdaBoost models are as follows:')
        print('   (1) AdaBoostRegressor\n   (2) AdaBoostClassifier')
        exit(1)
...
model_name = "AdaBoostRegressor"  #  "AdaBoostRegressor" or "AdaBoostClassifier"
...
pipe_params = {}
pipe_params['abr__loss'] = optuna.distributions.CategoricalDistribution(['linear', 'square', 'exponential'])
pipe_params['abr__base_estimator'] = optuna.distributions.CategoricalDistribution(
   [DecisionTreeRegressor(max_depth=3, max_features=5), 
    DecisionTreeRegressor(max_depth=3, max_features=6),
    DecisionTreeRegressor(max_depth=3, max_features=7), 
    DecisionTreeRegressor(max_depth=4, max_features=5),
    DecisionTreeRegressor(max_depth=4, max_features=6),
    DecisionTreeRegressor(max_depth=4, max_features=7)]
    )
pipe_params['abr__n_estimators'] = optuna.distributions.IntUniformDistribution(40,120,5)
pipe_params['abr__learning_rate'] = optuna.distributions.LogUniformDistribution(1e-3,1e1)
